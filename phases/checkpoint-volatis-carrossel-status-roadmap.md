...
> **Nota de recuperação (2026-07-21):** este arquivo vivia em
> `docs/referencias/volatis-content/carrossel-status-e-roadmap.md`, pasta que
> foi removida do repositório numa migração de privacidade anterior — os
> outros 9 arquivos da pasta continham dado sensível/comercial e a exclusão
> foi intencional; este era o único puramente técnico e saiu junto sem querer.
> Recuperado de uma worktree divergida (`claude/branch-cleanup-worktree-3671a9`)
> após auditoria confirmando que não existe em nenhum outro lugar do repo e
> não contém preço, nome de cliente ou dado sensível. **O conteúdo abaixo é de
> 2026-06-19 e pode estar defasado frente ao código atual** — trate como
> registro histórico de decisões (naming, schema N:N Marca↔Expert, mecanismo
> `reskin.ts`, fases da dinâmica de templates), não como o status vigente do
> Volatis. Para o estado atual, ver `phases/checkpoint-fase3-volatis-cockpit.md`.
